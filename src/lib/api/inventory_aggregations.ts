...
export async function getMissedDeliveriesInRange(params: {
	fromDate: string;
	toDate: string;
...
export async function getAllSubscribers(): Promise<{ id: string; center_name?: string }[]> {
	let page = 1;
...
export async function batchCreateAllocations(allocations: Array<{ center: string; date: string; quantity: number; added_by: string }>) {
	const results = await Promise.all(
		allocations.map(payload =>
			fetch(`${API_BASE_URL}/collections/inventory_allocations/records`, {
				method: 'POST',
